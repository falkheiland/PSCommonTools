Function Connect-Mstsc
{
  ...
  param (
    ...
    [Parameter(ParameterSetName = 'UserPassword', Mandatory, Position = 1)]
    [Alias('U')]
    [string]
    $User,

    [Parameter(ParameterSetName = 'UserPassword', Mandatory, Position = 2)]
    [Alias('P')]
    [SecureString]
    $Password,
  ...
  begin
  {
    ...
    switch ($true)
    {
      ...
      {
        $MstscArguments += ('/w:{0} ' -f $Width)
      }
      {
        $Height
      }
      {
        $MstscArguments += ('/h:{0} ' -f $Height)
      }
    }
  ...
  process
  {
    foreach ($Computer in $ComputerName)
    {
      ...
      $ProcessInfo.Arguments = ('/generic:TERMSRV/{0} /user:{1} /pass:{2}' -f $ComputerCmdkey, $User, ($Password))
      ...
      $ProcessInfo.Arguments = ('{0} /v {1}' -f $MstscArguments, $Computer)
      $ProcessInfo.WindowStyle = [Diagnostics.ProcessWindowStyle]::Normal
      $Process.StartInfo = $ProcessInfo
